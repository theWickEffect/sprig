...
import { mat4, V3 } from "../matrix/sprig-matrix.js";
import { createEmptyMesh, unshareProvokingVertices, } from "../meshes/mesh.js";
...
export const StatBarDef = EM.defineComponent("statBar", () => ({
    min: 0,
    max: 100,
    value: 80,
...
export function createMultiBarMesh({ width, length, centered, fullColor, missingColor, }) {
    ...
    mesh.quad.forEach((_, i) => {
        ...
        if (i <= part1Qidx)
            ...
            V3.copy(c, missingColor);
        mesh.colors.push(c);
    });
    mesh.surfaceIds = mesh.colors.map((_, i) => i + 1);
    const _mesh = unshareProvokingVertices(mesh, true);
...
EM.addEagerInit([StatBarDef], [], [], () => {
    ...
    EM.addSystem("renderStatBars", Phase.GAME_WORLD, [StatBarDef, RenderableDef], [RendererDef], (hs, res) => {
        const startPosIdx = 4; // TODO(@darzu): these magic numbers come from the mesh; better way?
        const lastPosIdx = startPosIdx + 3;
        for (let h of hs) {
            const { statBar } = h;
            // exit early if nothing to do
            if (statBar._lastRenderedValue === statBar.value)
                continue;
            statBar._lastRenderedValue = statBar.value;
            const percent = clamp(unlerp(statBar.min, statBar.max, statBar.value), 0, 1);
            const handle = h.renderable.meshHandle;
            assert(handle.mesh);
            assert(handle.mesh.dbgName === statBarMeshName, "StatBar entities must use createMultiBarMesh mesh");
            ...
            res.renderer.renderer.stdPool.updateMeshVertices(handle, mesh, startPosIdx, 4);
        }
    });
});
