...
import { V, mat3, quat, V3 } from "../matrix/sprig-matrix.js";
...
EM.addEagerInit([MastDef], [], [], () => {
    ...
    EM.addSystem("autoTurnMast", Phase.GAME_PLAYERS, [HasMastDef, WorldFrameDef], [InputsDef, WindDef], (es, res) => {
        for (let ship of es) {
            const mast = ship.hasMast.mast;
            if (!WorldFrameDef.isOn(mast))
                continue;
    ...
    EM.addSystem("mastPush", Phase.GAME_WORLD, [HasMastDef, WorldFrameDef, RotationDef, LinearVelocityDef], [], (es) => {
        for (let e of es) {
            // acceleration
            const direction = quat.fwd(e.world.rotation);
            const sailAccel = V3.scale(direction, e.hasMast.mast.mast.force * SAIL_ACCEL_RATE);
