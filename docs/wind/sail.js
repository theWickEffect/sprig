...
EM.addSystem("applyWindToSail", Phase.GAME_WORLD, [SailDef, WorldFrameDef], [WindDef], (es, res) => {
    for (let e of es) {
        ...
        if (e.sail._billowAmount < 0)
            e.sail._billowAmount = 0;
...
EM.addSystem("billowSails", Phase.GAME_WORLD, [SailDef, RenderableDef], [RendererDef], (es, { renderer }) => {
    for (let e of es) {
        if (Math.abs(e.sail._billowAmount - e.sail._lastSailBillow) < 0.01 &&
