...
import { BallMesh, GizmoMesh, PlaneMesh, UnitCubeMesh, } from "../meshes/mesh-list.js";
import { V2, V3, quat, V } from "../matrix/sprig-matrix.js";
...
import { scaleMesh, transformMesh, } from "../meshes/mesh.js";
...
import { getSizeFromAABB, isValidAABB } from "../physics/aabb.js";
...
export function drawBall(pos, size, color) {
...
export function drawPlane(opt) {
    const e = createObj([ColorDef], {
        color: opt.color ?? ENDESGA16.darkGray,
    });
    // TODO(@darzu): BROKEN!
    if ("center" in opt) {
        const scale = (opt.halfsize ?? 1.0) / 5;
        mixinObj(e, [PositionDef, RenderableConstructDef, ScaleDef, RotationDef], {
            position: [0, 0, 0],
...
export function addGizmoChild(parent, scale = 1, offset = [0, 0, 0]) {
...
export function drawGizmosForMat4(m, scale) {
...
export function createBoxForAABB(aabb) {
...
export function addColliderDbgVis(ent) {
    addColliderDbgVisForCollider(ent.collider);
    function addColliderDbgVisForCollider(c) {
        if (c.shape === "AABB") {
