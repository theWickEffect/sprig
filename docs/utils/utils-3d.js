import { V2, V3, quat, mat4, V, } from "../matrix/sprig-matrix.js";
...
export function randNormalVec3_cheap(out) {
    // NOTE: this isn't evenly distributed along a sphere; it's in a box
    const res = V3.set(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5, out ?? V3.tmp());
    V3.norm(res, res);
    return res;
}
export function randNormalVec3(out) {
    const res = V3.set(randFromNormalDist(), randFromNormalDist(), randFromNormalDist(), out ?? V3.tmp());
    V3.norm(res, res);
    return res;
}
...
export function jitterVec3(v, d, out) {
    out = out ?? V3.tmp();
...
export function randVec3OfLen(r = 1, out) {
    out = out ?? V3.tmp();
    randNormalVec3(out);
    V3.scale(out, r, out);
    return out;
...
const __t1 = V3.mk();
const __t2 = V3.mk();
export function quatFromUpForward_OLD(out, up, forwardish) {
...
export function signedAreaOfTriangle3(a, b, c) {
    const ab = V3.sub(b, a);
    const ac = V3.sub(c, a);
...
target) {
    const toward = V3.norm([target[0] - pos[0], 0, target[2] - pos[2]], __angleBetweenXZTmp0);
