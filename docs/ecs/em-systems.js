...
export function createEMSystems() {
    const allSystemsByName = new Map();
    const activeSystemsById = new Map();
    const systemNamesByPhase = toMap(PhaseValueList, (n) => n, (_) => []);
    const sysStats = {};
    // QUERY SYSTEM
    ...
    const _systemsToEntities = new Map();
    // NOTE: _entitiesToSystems is only needed because of DeadDef
    const _entitiesToSystems = new Map();
    const _systemsToComponents = new Map();
    const _componentToSystems = new Map();
    let _currentRunningSystem = undefined;
    ...
    function callSystems() {
        if (DBG_SYSTEM_ORDER) {
            ...
            if (_dbgLastSystemLen !== newTotalSystemLen ||
    ...
    function dbgGetSystemsForEntity(id) {
    ...
    function addSystem(name, phase, cs, rs, callback) {
        ...
        const seenAllCmps = (sys.cs ?? []).every((c) => _entities.seenComponents.has(c.id));
        ...
        if (seenAllCmps && seenAllRes) {
            ...
            //  added/removed
            _init.addEagerInit(sys.cs ?? [], sys.rs, [], () => {
                activateSystem(sys);
            }, `sysinit_${sys.name}`);
        }
        return sys;
        ...
        if (cs) {
            for (let c of cs) {
                if (!_componentToSystems.has(c.name))
                    ...
                    _componentToSystems.get(c.name).push(id);
            }
            _systemsToComponents.set(id, cs.map((c) => c.name));
        }
        for (let e of es) {
    ...
    function tryCallSystem(s) {
        ...
        let es;
        if (s.cs) {
            assertDbg(_systemsToEntities.has(s.id), `System ${s.name} doesn't have a query cache!`);
            es = _systemsToEntities.get(s.id);
        }
        else {
            es = [];
        }
        ...
        sysStats[s.name].maxCallTime = Math.max(sysStats[s.name].maxCallTime, thisCallTime);
        return true;
    }
    ...
    function whyIsntSystemBeingCalled(name) {
    ...
    function _notifyNewEntity(e) {
        _entitiesToSystems.set(e.id, []);
    }
    function _notifyAddComponent(e, def) {
        const id = e.id;
        // update query caches
    ...
    function _notifyRemoveComponent(e, def) {
        const id = e.id;
        // update query cache
        const systems = _componentToSystems.get(def.name);
        for (let sysId of systems ?? []) {
            if (sysId === _currentRunningSystem?.id &&
                !_currentRunningSystem.flags.allowQueryEdit)
                console.warn(`Removing component '${def.name}' while running system '${_currentRunningSystem.name}'` +
...
//# sourceMappingURL=em-systems.js.map