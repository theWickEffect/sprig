import { DBG_INIT_CAUSATION, DBG_VERBOSE_INIT_CALLSITES, DBG_VERBOSE_INIT_SEQ, } from "../flags.js";
...
export function createEMInit() {
    const initFnMsStats = new Map();
    const allInits = new Map();
    let _nextInitFnId = 1;
    // INIT SYSTEM
    ...
    const pendingLazyInitsByProvides = new Map();
    const pendingEagerInits = [];
    const startedInits = new Map();
    // TODO(@darzu): how can i tell if the event loop is running dry?
    // TODO(@darzu): EXPERIMENT: returns madeProgress
    function progressInitFns() {
    ...
    const _dbgInitBlameLn = new Map();
    function addInit(reg) {
        ...
        if (reg.eager) {
            ...
            for (let p of reg.provideRs) {
                assert(!pendingLazyInitsByProvides.has(p.id), `Resource: '${p.name}' already has an init fn!`);
    ...
    const _runningInitStack = [];
    let _lastInitTimestamp = -1;
    async function runInitFn(init) {
        ...
        for (let res of init.provideRs)
            assert(res.name in _resources.resources, `Init fn failed to provide: ${res.name}`);
    ...
    function addLazyInit(requireRs, provideRs, callback, name // TODO(@darzu): make required?
    ) {
    ...
    function addEagerInit(requireCompSet, requireRs, provideRs, callback, name // TODO(@darzu): make required?
    ) {
        const id = _nextInitFnId++;
        const reg = {
            requireCompSet,
    ...
    function summarizeInitStats() {
        ...
        const initsAndTimes = inits.map((reg) => [reg, initFnMsStats.get(reg.id)]);
...
//# sourceMappingURL=em-init.js.map