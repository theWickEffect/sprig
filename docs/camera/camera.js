...
import { transformCameraViewForWebGPUsNDC, transformYUpModelIntoZUp, } from "./basis.js";
...
export function getCameraSettings(e) {
    return {
...
export function applyCameraSettings(e, s) {
...
export function getCameraSettingsCodeStr(s) {
    return `
...
EM.addLazyInit([], [CameraDef], () => {
    ...
    EM.addSystem("cameraFollowTarget", Phase.PRE_RENDER, [CameraFollowDef], [CameraDef], (cs, res) => {
        ...
        if (target) {
            ...
            quat.fromYawPitchRoll(target.cameraFollow.yawOffset, target.cameraFollow.pitchOffset, 0, res.camera.rotationOffset);
        }
        else {
    ...
    EM.addSystem("retargetCamera", Phase.PRE_RENDER, null, [CameraDef], function ([], res) {
        ...
        if (prevTarget && newTarget) {
            if (VERBOSE_CAMERA)
                console.log(`retargetting camera`);
...
EM.addLazyInit([], [CameraComputedDef], () => {
    ...
    EM.addSystem("updateCameraView", Phase.RENDER_PRE_DRAW, null, [CameraComputedDef, CameraDef, MeDef, CanvasDef], (_, resources) => {
        ...
        if (targetEnt) {
            const computedTranslation = V3.add(frame.position, camera.targetPositionError);
        ...
        const computedCameraTranslation = V3.add(camera.positionOffset, camera.cameraPositionError);
        ...
        if (VERBOSE_CAMERA) {
            dbgLogOnce(`computedCameraTranslation: ${vec3Dbg(computedCameraTranslation)}`);
            dbgLogOnce(`computedCameraRotation: ${quatDbg(computedCameraRotation)}`);
        }
