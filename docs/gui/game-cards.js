...
export async function initCardsGame() {
    ...
    //   postProcess,
    // ];
    EM.addSystem("gameCardsPipelines", Phase.GAME_WORLD, null, [RendererDef, DevConsoleDef], (_, res) => {
        res.renderer.pipelines = [
            stdMeshPipe,
            alphaRenderPipeline,
            outlineRender,
    ...
    if (DBG_GIZMOS)
        addWorldGizmo(V(-PANEL_W * 0.5, -PANEL_H * 0.5, 0));
    if (DBG_3D) {
...
//# sourceMappingURL=game-cards.js.map