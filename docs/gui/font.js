...
import { RendererDef, RenderableDef, RenderableConstructDef, } from "../render/renderer-ecs.js";
import { svgToLineSeg, compileSVG } from "../utils/svg.js";
...
export const pipeFontLineRender = CY.createRenderPipeline("pipeFontLineRender", {
    globals: [],
    shader: () => `
  ${shader_fontLine}
  `,
...
const pipeFontLineSdfExample = createRenderTextureToQuad("pipeFontLineSdfExample", fontJfa.sdfTex, fontLineSdfExampleTex, -1, 1, -1, 1, true, () => `
    ...
    // }
  `).pipeline;
export const FontDef = defineResourceWithInit("font", [RendererDef], async (res) => {
    return new Promise(async (resolve, reject) => {
        ...
        for (let i = 0; i < CHARS.length; i++) {
            ...
            const ent = EM.mk();
            EM.set(ent, RenderableConstructDef, mesh, true, undefined, FONT_JFA_MASK, lineMeshPoolPtr);
            ...
            promises.push(EM.whenEntityHas(ent, RenderableDef, WorldFrameDef, PositionDef, LineUniDef
            // ColorDef
            ));
        }
