...
export class Peer {
    ...
    async handleServerMessage(msg) {
        ...
        switch (msg.type) {
            case ServerMessageType.Offer: {
                ...
                if (VERBOSE_NET_LOG)
                    console.log(`connectionId: ${connectionId}, remotePeerId: ${remotePeerId}`);
            ...
            case ServerMessageType.Answer: {
                if (VERBOSE_NET_LOG)
                    console.log(`ServerMessageType.Answer`);
                let peerConnection = this.connections[msg.payload.connectionId];
                if (!peerConnection) {
                    throw `ICE candidate for unknown connection ${msg.payload.connectionId}`;
                }
    ...
    async connect(peerId, reliable) {
        if (VERBOSE_NET_LOG)
            console.log(`new connet(${peerId}, ${reliable})`);
