...
import { centroid, quatFromUpForward_OLD, randNormalVec3, } from "../utils/utils-3d.js";
...
EM.addEagerInit([WoodStateDef], [], [], () => {
    ...
    EM.addSystem("woodHealth", Phase.GAME_WORLD, [WoodStateDef, WorldFrameDef, WoodHealthDef, RenderableDef], [RendererDef, SplinterPoolsDef], (es, res) => {
            ...
            w.woodState.boards.forEach((board, bIdx) => {
                ...
                board.segments.forEach((seg, sIdx) => {
                    ...
                    if (!h.broken && h.health <= 0) {
                        if (DBG_WOOD_DMG)
                            console.log(`breaking ${w.id}.woodHealth.boards[${bIdx}][${sIdx}]`);
                        h.broken = true;
                        ...
                        // TODO(@darzu): MOVE into wood-splinters.ts ?
                        {
                            ...
                            if (ColorDef.isOn(w))
                                V3.copy(splinter.color, w.color);
...
export function reserveSplinterSpace(wood, maxSplinters) {
    ...
    range(maxSplinters * _vertsPerSplinter).forEach((_) => wood.mesh.pos.push(V3.mk()));
    range(maxSplinters * _trisPerSplinter).forEach((_) => wood.mesh.tri.push(V3.mk()));
    range(maxSplinters * _quadsPerSplinter).forEach((_) => wood.mesh.quad.push(V4.mk()));
...
export function getBoardsFromMesh(m) {
    ...
    function createBoard(startQi) {
        ...
        isFirstLoop = false) {
            ...
            const nextLoop = V4.clone(nextLoop_);
            ...
            if (segQis.length === 5) {
                ...
                if (endQuads.length === 1) {
                    ...
                    const sideQuads = V4.clone(segQis.filter((qi) => qi !== endQuad));
                    seg = {
                        localAABB: aabb,
                        midLine: mid,
                        areaNorms,
