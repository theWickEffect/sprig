...
import { cloneMesh, getAABBFromMesh, transformMesh, } from "../meshes/mesh.js";
...
export async function initShipyardGame() {
    ...
    transformMesh(groundMesh, mat4.fromRotationTranslationScale(quat.IDENTITY, [0, 0, -4], [20, 20, 2]));
    ...
    // TIMBER
    const timber = EM.mk();
    const { timberState, timberMesh, ribCount, ribSpace, ribWidth, ceilHeight, floorHeight, floorLength, floorWidth,
    // } = createSpaceBarge();
     } = createLD53Ship();
    // TODO(@darzu): remove
    // const ribCount = 10;
    ...
    EM.addSystem("ld51Ghost", Phase.GAME_WORLD, [GhostDef, WorldFrameDef, ColliderDef], [InputsDef, HasFirstInteractionDef], async (ps, { inputs }) => {
        ...
        if (inputs.lclick) {
            ...
            fireBullet(1, firePos, fireDir, 0.05, 0.02, 3 * 0.00001, ballHealth, V3.FWD);
        }
