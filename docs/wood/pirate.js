import { BLACK, CannonLD51Mesh, } from "../meshes/mesh-list.js";
...
import { transformMesh, getAABBFromMesh, normalizeMesh, } from "../meshes/mesh.js";
        ...
        for (let p of ps) {
            ...
            if (doFire) {
                ...
                if (WorldFrameDef.isOn(c)) {
                    ...
                    fireBullet(2, c.world.position, c.world.rotation, 0.05, 0.02, 3 * 0.00001, ballHealth, V3.FWD);
                }
            }
        }
    });
}
export const PiratePoolDef = EM.defineResource("piratePool", (p) => p);
    ...
    const piratePool = createEntityPool({
        ...
        onSpawn: (p) => {
            ...
            EM.whenEntityHas(timber, RenderableDef).then((timber2) => {
                res.renderer.renderer.stdPool.updateMeshQuadInds(timber2.renderable.meshHandle, timber.woodState.mesh, 0, timber.woodState.mesh.quad.length);
            });
            // undead
