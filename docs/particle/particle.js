...
import { ParticleStruct, particleQuadInds, particleQuadVert, } from "../render/pipelines/std-particle.js";
import { litTexturePtr, mainDepthTex, sceneBufPtr, } from "../render/pipelines/std-scene.js";
...
//   stop: () => void;
// }
function createParticleSystem(desc) {
    ...
    */
    ...
    const pipeRender = CY.createRenderPipeline(`pipeParticleRender_${desc.name}`, {
        globals: [sceneBufPtr],
        meshOpt: {
    ...
    const pipeUpdate = CY.createComputePipeline(`pipeParticleUpdate_${desc.name}`, {
        shaderComputeEntry: "main",
        shader: (shaders) => `const numParticles: u32 = ${maxParticles};
...
export const ParticleDef = defineResourceWithInit("particles", [RendererDef], ({ renderer }) => {
    // TODO(@darzu): IMPL
    EM.addSystem("updateEmitters", Phase.POST_GAME_WORLD, [EmitterDef, WorldFrameDef], [TimeDef], (es, res) => {
        const systems = es.reduce((p, n) => (n.emitter.system ? p.add(n.emitter.system) : p), new Set() // TODO(@darzu): PERF: reuse
        );
        const events = [];
        for (let e of es) {
            if (!e.emitter.system)
            ...
            while (e.emitter.pulseNum.length)
                amount += e.emitter.pulseNum.pop();
            if (amount)
                events.push({
...
export const EmitterDef = EM.defineComponent("emitter", () => {
    const e = {
        system: undefined,
