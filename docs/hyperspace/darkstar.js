...
export const { DarkStarPropsDef, DarkStarLocalDef, createDarkStarNow } = defineNetEntityHelper({
    ...
    updateProps: (p, pos, color, orbiting, orbitalAxis) => {
        ...
        if (orbiting)
            V3.copy(p.orbiting, orbiting);
        if (orbitalAxis)
            V3.copy(p.orbitalAxis, orbitalAxis);
        return p;
    },
...
export function registerDarkstarSystems() {
    ...
    EM.addSystem("darkStarOrbit", Phase.GAME_WORLD, [DarkStarPropsDef, PositionDef, AuthorityDef], [MeDef, HSGameStateDef], (es, res) => {
        ...
        for (let star of es) {
            ...
            */
            const movementDirection = V3.cross(toCenter, star.darkStarProps.orbitalAxis);
            V3.norm(movementDirection, movementDirection);
            V3.add(star.position, V3.scale(movementDirection, DARKSTAR_SPEED, movementDirection), star.position);
