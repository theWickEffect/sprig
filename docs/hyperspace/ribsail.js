    ...
    EM.addSystem(`updateRibSail`, Phase.GAME_WORLD, [RibSailLocalDef, RenderableDef], [RendererDef], (cs, res) => {
        for (let sail of cs) {
            ...
            sail.ribSailLocal.ribs.forEach((ribRef, i) => {
                const rib = ribRef();
                quat.rotX(quat.IDENTITY, sail.ribSailLocal.pitch * (1 - i / RIB_COUNT), rib.rotation);
            });
...
export function getSailMeshArea(verts) {
    ...
    return (signedAreaOfTriangle(V(verts[0][1], verts[0][2]), V(verts[1][1], verts[1][2]), V(verts[2][1], verts[2][2])) * RIB_COUNT);
}
export function sailForceAndSignedArea(sail, starPos) {
    const viewProjMatrix = positionAndTargetToOrthoViewProjMatrix(mat4.tmp(), starPos, sail.world.position);
    ...
    const sailNormal = V3.cross(V3.sub(worldVerts[1], worldVerts[0]), V3.sub(worldVerts[2], worldVerts[0]));
