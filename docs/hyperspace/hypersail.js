...
const SailColorDef = EM.defineComponent("sailColor", () => V3.mk(), (p, color) => (color ? V3.copy(p, color) : p));
    ...
    EM.addSystem("sail", Phase.GAME_PLAYERS, [HsShipPropsDef, UVShipDef, WorldFrameDef, AuthorityDef], [MeDef, HSGameStateDef], (es, res) => {
        ...
        for (let ship of es) {
            ...
            for (let sail of sails) {
                ...
                //   }, realArea ${realArea}, accel ${accel}`
                // );
                V3.lerp(DEFAULT_SAIL_COLOR, V3.norm(star.color), clamp((accel / realArea) * 5000, 0, 1), sail.color);
