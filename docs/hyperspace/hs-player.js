...
export const PlayerHsPropsDef = defineSerializableComponent("hsPlayerProps", () => ({ location: V3.mk() }), (p, loc) => {
    if (loc)
        V3.copy(p.location, loc);
    return p;
}, (c, writer) => {
    ...
    EM.addSystem("stepHsPlayers", Phase.GAME_PLAYERS, [
        ...
        for (let p of players) {
            ...
            if (cheat && inputs.keyClicks["r"]) {
                // create our ray
                const r = {
                    org: V3.add(p.world.position, V3.scale(V3.mul(facingDir, p.world.scale), 3.0), V3.mk()),
                    dir: facingDir,
                };
                playerShootRay(r);
            }
        ...
        for (let p of players) {
            ...
            if (!parent) {
                ...
                if (ship) {
                    ...
                    const endPos = V3.clone([
                        3.5 * (evenPlayer ? 1 : -1),
                        shipY + pFeetToMid + 1,
                        Math.floor((res.me.pid - 1) / 2) * 4 - 10,
                    ]);
                    const startPos = V3.add(endPos, [0, 200, 0], 
                    // V3.tmp(),
                    V3.mk());
