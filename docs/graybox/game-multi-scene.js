...
export async function initMultiSceneGame() {
    ...
    EM.addSystem("repeatSpawn", Phase.GAME_WORLD, null, [TimeDef, RendererDef], (_, res) => {
        if (res.time.step % (60 * 3) === 0) {
            res.renderer.renderer.submitPipelines([], [cloudBurstSys.pipeInit]);
        }
    });
    // TODO(@darzu): change particle color based on current canvas!
    const blurbs = [...document.getElementsByClassName("demo-and-blurb")].map((b) => {
        assert(b.tagName === "DIV", `Element isn't a div, it's a: ${b.tagName}`);
        return b;
    });
    let _hoverIdx = 0;
    let _activeIdx = 0;
    blurbs.forEach((b, i) => (b.onmousemove = () => {
        _hoverIdx = i;
    }));
    EM.addSystem("canvasSwitch", Phase.GAME_WORLD, [], [CanvasDef, InputsDef], (_, res) => {
...
//# sourceMappingURL=game-multi-scene.js.map