...
function testObjectTS2() {
    ...
    const CannonObj = O_def("cannon", // defines a new tag component
    PositionDef);
    // defines a component "sailable"
    const SailableDef = O_comp({ sailable: T() });
    // defines an object w/ 2 components
    const CargoObj = O_def(ColorDef, ScaleDef);
    const ShipObj = O_def({ ship2: T() }, // defines a component
    { position: PositionDef }, RenderableConstructDef, SailableDef, {
        child: {
            mast: [
                "mast2",
                ScaleDef,
    ...
    const rudder = O_mk([PositionDef, RotationDef], [1, 1, 1], undefined);
...
//# sourceMappingURL=objects_crazy.js.map