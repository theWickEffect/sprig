...
import { BallMesh, CannonMesh, CubeMesh, PlaneMesh, } from "../meshes/mesh-list.js";
import { mkCubeMesh } from "../meshes/primatives.js";
import { ParametricDef, copyParamateric, createParametric, createPathFromParameteric, } from "../motion/parametric-motion.js";
...
import { ChildrenDef, createObj, defineObj, mixinObj, } from "../ecs/em-objects.js";
...
function cannonFireCurve(frame, speed, out) {
...
function launchBall(params, team) {
    ...
    if (DBG_TRAILS)
        EM.set(ball, SketchTrailDef);
    return ball;
}
EM.addSystem("ballHitWater", Phase.GAME_WORLD, [CannonBallDef, PositionDef], [], (es) => {
    for (let e of es) {
        if (e.position[2] < 0) {
...
function mkDotPath(dotsRes, len, color, size) {
    const path = range(len).map((_) => ({
        pos: V3.mk(),
        rot: quat.mk(),
    }));
...
export async function initGrayboxShipArena() {
    ...
    */
    ...
    EM.addSystem("shipArenaPipelines", Phase.GAME_WORLD, [], [RendererDef, GraphicsSettingsDef, DevConsoleDef], (_, res) => {
        // renderer
        res.renderer.pipelines = [];
        res.renderer.pipelines.push(...shadowPipelines, stdMeshPipe, renderDots, alphaRenderPipeline, outlineRender, deferredPipeline, linePipe, pointPipe, stdGridRender, postProcess);
    });
    ...
    // grid
    const grid = createObj([RenderableConstructDef, PositionDef, ScaleDef, ColorDef], {
    ...
    const _launchParam = createParametric();
    // let _imATmp = V3.tmp();
    EM.addSystem("controlShip", Phase.GAME_PLAYERS, [ShipDef, PlayerShipDef, HasRudderDef, HasMastDef, CameraFollowDef], [InputsDef, CanvasDef, RendererDef, TimeDef], (es, res) => {
        // TODO(@darzu): CANVAS
        ...
        if (!aiming) {
            ...
            ship.cameraFollow.pitchOffset = clamp(ship.cameraFollow.pitchOffset, -PI * 0.5, 0);
            ship.cameraFollow.yawOffset = wrap(ship.cameraFollow.yawOffset, -PI, PI);
        }
        // which cannons?
        ...
        if (aiming) {
            for (let c of cannons) {
                ...
                c.yawpitch.yaw =
                    clamp(c.yawpitch.yaw - c.cannon2.baseYaw, -CANNON_MAX_YAW, CANNON_MAX_YAW) + c.cannon2.baseYaw;
            }
        }
        for (let c of cannons) {
    ...
    onCollides([CannonBallDef], [EnemyDef, ShipDef], [], (ball, ship) => {
        if (ball.cannonBall.team !== PLAYER_TEAM)
            return;
    ...
    onCollides([CannonBallDef], [PlayerShipDef, ShipDef], [], (ball, ship) => {
        if (ball.cannonBall.team !== ENEMY_TEAM)
            return;
        ship.ship.healthBar.statBar.value -= 10;
        EM.set(ball, DeletedDef);
    });
...
function doDeadObjAndChildren(e) {
...
function createShip(opts) {
    ...
    EM.whenEntityHas(mast, ColliderDef, PositionDef).then((mast) => {
        ...
        sock.position[2] =
            mast.position[2] + mast.collider.aabb.max[2];
...
function getDirsToTan(src, trg, trgRad, outL, outR) {
    const srcToTrg = V3.sub(trg, src);
    const perpR = [srcToTrg[1], -srcToTrg[0], 0];
...
async function initEnemies() {
    const player = await EM.whenSingleEntity(PlayerShipDef, PositionDef, OBBDef, WorldFrameDef);
    const { dots } = await EM.whenResources(DotsDef);
    const trgDots = DBG_ENEMY_MOV ? dots.allocDots(10) : undefined;
    const steerFreq = 20;
    EM.addSystem("enemySailFindTarget", Phase.GAME_WORLD, [EnemyDef, PositionDef, RotationDef, HasRudderDef], [TimeDef], (es, res) => {
        // run once every 20 frames
        if (res.time.step % steerFreq !== 0)
            return;
        for (let e of es) {
            const trgL = V3.tmp();
            const trgR = V3.tmp();
            getDirsToTan(e.position, player.position, enemyAttackRadius, trgL, trgR);
    ...
    EM.addSystem("enemySailToward", Phase.GAME_WORLD, [EnemyDef, PositionDef, RotationDef, HasRudderDef, HasMastDef], [TimeDef], (es, res) => {
    ...
    EM.addSystem("enemyAttack", Phase.GAME_WORLD, [EnemyDef, ShipDef, PositionDef, RotationDef, WorldFrameDef], [TimeDef], (es, res) => {
        ...
        for (let e of es) {
            // reload
            if (res.time.time < e.enemy.lastFireMs + e.enemy.reloadMs)
                continue;
            // aim
            ...
            if (!WorldFrameDef.isOn(centerCannon))
                continue;
            const defaultWorldRot = quat.yaw(e.world.rotation, centerCannon.cannon2.baseYaw);
            ...
            for (let c of cannons) {
                ...
                const firePara = cannonFireCurve({ position: c.world.position, rotation }, cannonBallSpeed, _enemyLaunchParam);
...
//# sourceMappingURL=graybox-ship-arena.js.map