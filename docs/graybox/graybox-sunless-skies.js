...
export async function initGrayboxSunless() {
    ...
    const { mesh_cube, mesh_hex } = await EM.whenResources(CubeMesh.def, HexMesh.def);
    // dbg ghost
    if (DBG_GHOST) {
...
async function createWorld() {
    const { mesh_cube, mesh_unitCube } = await EM.whenResources(CubeMesh.def, UnitCubeMesh.def);
    ...
    function createWall(pos, vertical, length) {
        const wall = EM.mk();
        EM.set(wall, PositionDef, pos);
        EM.set(wall, RenderableConstructDef, mesh_cube.proto);
        if (vertical)
            EM.set(wall, ScaleDef, V(wallWidth, length, wallHeight));
        else
            EM.set(wall, ScaleDef, V(length, wallWidth, wallHeight));
    ...
    function createDbgPath(pos, horizontal) {
        const path = EM.mk();
        ...
        if (horizontal)
            ...
            EM.set(path, ScaleDef, V(0.2, gridHalfScale, 0.2));
    ...
    for (let yi = 0; yi < gridWidth; yi++) {
        for (let xi = 0; xi < gridWidth - 1; xi++) {
            ...
            if (!hasEdge)
                createWall(pos, true, gridHalfScale);
            if (DBG_GRID && hasEdge)
                createDbgPath(pos, true);
        }
    }
    for (let yi = 0; yi < gridWidth - 1; yi++) {
        for (let xi = 0; xi < gridWidth; xi++) {
            ...
            if (!hasEdge)
                createWall(pos, false, gridHalfScale);
            if (DBG_GRID && hasEdge)
                createDbgPath(pos, false);
        }
    }
    // outer walls
    [
    ...
    {
        const floor = EM.mk();
        EM.set(floor, ScaleDef, V(gridScale * gridWidth, gridScale * gridWidth, wallHeight));
    ...
    for (let [xi, yi, dir] of docks) {
        ...
        const node = EM.mk();
        EM.set(node, RenderableConstructDef, mesh_cube.proto);
        EM.set(node, ScaleDef, V(vert ? dockWidth : dockLen, vert ? dockLen : dockWidth, dockHeight));
        EM.set(node, PositionDef, V(xi * gridScale, yi * gridScale, 0));
...
EM.addEagerInit([SunlessShipDef], [], [], (_) => {
    EM.addSystem("moveSunlessShip", Phase.POST_GAME_PLAYERS, [SunlessShipDef, RotationDef, LinearVelocityDef], [TimeDef], (ships, res) => {
        for (let e of ships) {
            // acceleration
            const speed = e.sunlessShip.speed * res.time.dt;
            const rotatedAccel = V3.tQuat(e.sunlessShip.localAccel, e.rotation);
            // turn
            quat.yaw(e.rotation, e.sunlessShip.localYaw * e.sunlessShip.yawSpeed, e.rotation);
            // reset turn
...
EM.addEagerInit([SunlessPlayerDef], [CubeMesh.def], [], ({ mesh_cube }) => {
    EM.addSystem("moveSunlessPlayerShip", Phase.GAME_PLAYERS, [SunlessPlayerDef, SunlessShipDef], [InputsDef, TimeDef], (ships, res) => {
        if (!ships.length)
            return;
    ...
    EM.addSystem("controlSunlessShip", Phase.GAME_PLAYERS, [SunlessPlayerDef, RotationDef, LinearVelocityDef, PositionDef], [InputsDef, TimeDef], (ships, res) => {
        if (!ships.length)
            return;
        assert(ships.length === 1);
        const ship = ships[0];
        if (res.inputs.keyClicks[" "]) {
            ...
            V3.add(ship.linearVelocity, bullet.linearVelocity, bullet.linearVelocity);
            // kickback
    ...
    onCollides([BulletDef, LinearVelocityDef], [
        EnemyDef,
        // HealthDef,
        LinearVelocityDef,
    ], [], (bullet, enemy) => {
...
//# sourceMappingURL=graybox-sunless-skies.js.map