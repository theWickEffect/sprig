...
EM.addSystem("updateGlitchMeshIndices", Phase.GAME_WORLD, [RenderableDef, GlitchDef], [TimeDef], (es, res) => {
    ...
    for (let e of es) {
        ...
        if (pool.ptr.prim === "tri") {
            ...
            if (m.tri.length)
                pool.updateMeshTriInds(e.renderable.meshHandle, m);
            if (m.quad.length)
                pool.updateMeshQuadInds(e.renderable.meshHandle, m);
        }
