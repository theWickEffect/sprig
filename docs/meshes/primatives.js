...
import { mat4, quat, V, V2, V3, V4 } from "../matrix/sprig-matrix.js";
...
export function mkRectMesh(xLen, yLen, zLen) {
    let hx = xLen / 2;
    let hy = yLen / 2;
    let hz = zLen / 2;
    let result = {
...
export const mkLineSegs = (segNum) => ({
    dbgName: "lineSegs",
...
export function makePlaneMesh(x1, x2, y1, y2) {
    const res = {
    ...
    // create each quad
    {
        ...
        for (let y = 0; y < height - 1; y++) {
            for (let x = 0; x < width - 1; x++) {
                V4.set(idx(x + 1, y), //
                idx(x + 1, y + 1), idx(x, y + 1), idx(x, y), mesh.quad[i]);
                i++;
                if (doubleSided) {
                    V4.set(idx(x, y), //
                    idx(x, y + 1), idx(x + 1, y + 1), idx(x + 1, y), mesh.quad[i]);
                    i++;
                }
...
export function mkOctogonMesh() {
    ...
    }, mat4.fromRotationTranslationScaleOrigin(quat.IDENTITY, [-1.5, -1.5, 0], [0.2, 0.2, 0.2], [1.5, 1.5, 0]));
}
export function mkHalfEdgeQuadMesh() {
    return transformMesh({
        ...
        quad: [V(0, 1, 2, 3)],
        colors: [V(0, 0, 0)],
    }, mat4.fromRotationTranslationScaleOrigin(quat.IDENTITY, [0, -1.5, 0], [0.2, 0.4, 0.2], [0, 1.5, 0]));
}
