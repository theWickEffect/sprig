...
export function transformRigging(out, tm) {
    /*
    ...
    */
    ...
    out.poseRot.forEach((qs) => qs.forEach((q) => quat.mul(tq, quat.mul(q, tqi, q), q)));
...
export function unshareProvokingVerticesWithMap(input, strictNoAdd = false) {
    const pos = [...input.pos];
    ...
    input.tri.forEach(([i0, i1, i2]) => {
        if (!provoking[i0]) {
            ...
            if (strictNoAdd)
                throw new Error(`Cannot unshare provoking for '${input.dbgName}' mesh w/o adding vertex for triangle.`);
            const i3 = pos.length;
            pos.push(input.pos[i0]);
            posMap.set(i3, i0);
            if (uvs)
...
export function unshareProvokingVertices(input, strictNoAdd = false) {
    const { mesh, posMap, provoking } = unshareProvokingVerticesWithMap(input, strictNoAdd);
    return mesh;
}
...
export function mergeMeshes(...rs) {
    ...
    const m = {
        ...
        dbgName: rs.some((r) => r.dbgName)
            ...
            : undefined,
        usesProvoking: rs.every((r) => r.usesProvoking)
            ? true
            : undefined,
    }; // TODO(@darzu): Ugh these casts. Gotta impl this right
    for (let r of rs) {
        m.pos = [...m.pos, ...r.pos];
        m.tri = [
            ...m.tri,
        ...
        m.quad = [
            ...m.quad,
            ...r.quad.map((t) => V4.clone([t[0] + posIdx, t[1] + posIdx, t[2] + posIdx, t[3] + posIdx])),
        ];
        if (m.lines)
            m.lines = [
                ...m.lines,
