...
import { V3, quat, mat4, V, tV } from "../matrix/sprig-matrix.js";
...
import { cloneMesh, getAABBFromMesh, mapMeshPositions, mergeMeshes, scaleMesh, scaleMesh3, } from "../meshes/mesh.js";
...
export async function initGJKSandbox() {
    ...
    // sun
    const sunlight = createObj([PointLightDef, PositionDef, RenderableConstructDef], {
        pointLight: {
            constant: 1.0,
    ...
    // );
    // world gizmo
    const worldGizmo = createObj([PositionDef, ScaleDef, RenderableConstructDef], [[0, 0, 0], [1, 1, 1], [GizmoMesh]]);
    // ghost mesh
...
async function obbTest(g) {
    ...
    const cubes = _cubes.map((c) => {
        mixinObj(c, [ColorDef, WorldFrameDef, ColliderDef, OBBDef], [
            V(0.1, 0.1, 0.1),
            undefined,
            {
                shape: "AABB",
                solid: false,
                aabb: getAABBFromMesh(c.renderableConstruct.meshOrProto),
            },
            undefined,
        ]);
        return c;
...
function checkerMesh(m, xyz) {
    ...
    const scale3 = tV(newSize[0] / size[0], newSize[1] / size[1], newSize[2] / size[2]);
...
async function gjkTest(g, ghostGameMesh) {
    ...
    EM.addSystem("checkGJK", Phase.GAME_WORLD, null, [], (_, {}) => {
        ...
        // TODO(@darzu):
        let playerShape = createWorldShape(ghostGameMesh, g.position, g.rotation, lastPlayerPos);
        let backTravelD = 0;
        for (let i = 0; i < gjkEnts.length; i++) {
            // g.color[i] = 0.1;
            gjkEnts[i].color[i] = 0.1;
            let shapeOther = createWorldShape(gjkGameMeshes[i], gjkEnts[i].position, gjkEnts[i].rotation, lastWorldPos[i]);
            ...
            if (simplex &&
                ...
                shapeOther = createWorldShape(gjkGameMeshes[i], gjkEnts[i].position, gjkEnts[i].rotation, lastWorldPos[i]);
                playerShape = createWorldShape(res.allMeshes.cube, g.position, g.rotation, lastPlayerPos);
