    ...
    EM.addSystem("physicsInit", Phase.PRE_PHYSICS, [ColliderDef, PositionDef], [PhysicsBroadCollidersDef], (objs, { _physBColliders }) => {
        for (let o of objs) {
            ...
            if (o.collider.shape === "AABB") {
                _phys.colliders.push(mkCollider(o.collider.aabb, o.id, parentId, o.collider.myLayer, o.collider.targetLayer));
            }
            else if (o.collider.shape === "Multi") {
                for (let c of o.collider.children) {
                    if (c.shape !== "AABB")
                        throw `Unimplemented child collider shape: ${c.shape}`;
                    _phys.colliders.push(mkCollider(c.aabb, o.id, parentId, o.collider.myLayer, o.collider.targetLayer));
                }
            }
            else {
        ...
        function mkCollider(selfAABB, oId, parentOId, myLayer, targetLayer) {
            ...
            for (let o of objs) {
                ...
                if (objDoesMov) {
                    ...
                    if (DBG_FIRST_FRAME_MOV && o._phys.dbgFirstFrame)
                        console.warn(`Object '${o.id}' is being moved ${vec3Dbg(refl)} by contact constraints on its first frame.` +
