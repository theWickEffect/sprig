...
import { V, V3, V4 } from "../matrix/sprig-matrix.js";
import { BallMesh, PlaneMesh } from "../meshes/mesh-list.js";
import { MeDef } from "../net/components.js";
import { PositionDef, ScaleDef } from "../physics/transform.js";
import { CY } from "../render/gpu-registry.js";
import { GAME_JFA_MASK, GRID_MASK, SUMMON_CIRCLE_MASK, } from "../render/pipeline-masks.js";
import { createGridComposePipelines } from "../render/pipelines/std-compose.js";
import { deferredPipeline } from "../render/pipelines/std-deferred.js";
import { stdGridRender } from "../render/pipelines/std-grid.js";
import { LineUniDef, lineMeshPoolPtr, linePipe, pointPipe, } from "../render/pipelines/std-line.js";
...
import { clamp, jitter, sum } from "../utils/math.js";
...
export async function initLd55() {
    ...
    if (!ALWAYS_JFA) {
        ...
        EM.addSystem("summonLineJfa", Phase.GAME_WORLD, [LineUniDef, RenderableDef], [RendererDef], (es, res) => {
            _frame++;
            if (_frame > 2 && lastSummonLinesLen === summonLines.length)
                return;
    ...
    EM.addSystem("ld55Pipelines", Phase.GAME_WORLD, null, [RendererDef, DevConsoleDef, TimeDef], (_, res) => {
        // renderer
    ...
    const { camera, me, renderer } = await EM.whenResources(CameraDef, MeDef, RendererDef);
    // camera
    ...
    // pedestal
    {
        ...
        EM.set(pedestal, RenderableConstructDef, pedestalMesh, true, undefined, SUMMON_CIRCLE_MASK);
    ...
    if (FIRST_CIRCLE) {
        ...
        const lines = svgToLineSeg(compileSVG([
            { i: "M", x: -radius, y: 0 },
        ...
        EM.whenEntityHas(e, RenderableDef).then((e) => summonLines.push(e.renderable.meshHandle));
    }
    const particlesPerDist = 10;
    const tsData = [];
    function linesToParametric(lines) {
    ...
    function particleTrailOnLines(lines) {
        const particleData = renderer.renderer.getCyResource(cloudBurstSys._data);
        assert(!!particleData, `missing particleData`);
        const param = linesToParametric(lines);
        const totalDist = param.len(1);
        let maxParticlesPerDist = Math.floor(cloudBurstSys.desc.maxParticles / totalDist);
        let actualParticlesPerDist = Math.min(maxParticlesPerDist, particlesPerDist);
        let numParticles = Math.floor(actualParticlesPerDist * totalDist);
        while (tsData.length < numParticles)
    ...
    const leftDot = createObj([PositionDef, ColorDef, RenderableConstructDef, ScaleDef], {
        position: [0, 0, 2],
        color: ENDESGA16.darkGreen,
        renderableConstruct: [BallMesh],
        scale: [0.2, 0.2, 1],
    });
    const rightDot = createObj([PositionDef, ColorDef, RenderableConstructDef, ScaleDef], {
        position: [0, 0, 2],
    ...
    let lineColorIdx = 0;
    EM.addSystem("updateStickDots", Phase.GAME_WORLD, null, [GamepadDef, SketcherDef], (_, { gamepad, sketcher }) => {
        // left chalk position
        ...
        // symmetry setting
        if (gamepad.btnClicks["left"])
            symmetry -= 1;
        else if (gamepad.btnClicks["right"])
            symmetry += 1;
        symmetry = clamp(symmetry, 1, 20);
        maxSymmetry = Math.max(maxSymmetry, symmetry);
        // color setting
        if (gamepad.btnClicks["up"])
            lineColorIdx += 1;
        if (gamepad.btnClicks["down"])
            lineColorIdx -= 1;
        ...
        for (let i = 0; i < symmetry; i++) {
            ...
            const right = V3.yaw([rightDot.position[0], rightDot.position[1], 0.1], a);
    ...
    EM.addSystem("summonMonsters", Phase.GAME_WORLD, null, [GamepadDef, InputsDef], (_, { gamepad, inputs }) => {
...
//# sourceMappingURL=game-ld55.js.map