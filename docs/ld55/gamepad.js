...
EM.addLazyInit([], [GamepadDef], () => {
    ...
    EM.addSystem("gamepad", Phase.READ_INPUTS, null, [GamepadDef], (_, { gamepad }) => {
        ...
        if (rawPad.buttons.length < xboxLayout.length) {
            dbgLogOnce("gamepadButtons", `assuming xbox layout, number of buttons: ${rawPad.buttons.length} vs. xbox num: ${xboxLayout.length}`, true);
            return;
        }
