...
import { mkRectMesh } from "../meshes/primatives.js";
...
import { SketcherDef, sketchLineSegs } from "../utils/sketch.js";
...
function getMonsterStats(stats) {
...
export function summonMonster(summonStats) {
    const stats = getMonsterStats(summonStats);
    const { bodyWidth, bodyLength, headSize, numPairsFeet, legLength, startHeight, footSpacing, footMarkFwdOffset, strideWidth, } = stats;
...
EM.addEagerInit([MonsterDef], [], [], () => {
    EM.addSystem("updateMonsters", Phase.GAME_WORLD, [MonsterDef, ...MonsterObj.opts.components], // TODO(@darzu): ABSTRACTION. Need better handling for this
    [TimeDef], (es, res) => {
    ...
    let firstFrameSeen = new Map();
    EM.addSystem("updateMonsterFeet", Phase.GAME_WORLD, [MonsterDef, ...MonsterObj.opts.components, WorldFrameDef], // TODO(@darzu): ABSTRACTION. Need better handling for this
    [TimeDef, SketcherDef], (es, res) => {
        for (let e of es) {
            ...
            for (let foot of e.monster.feet) {
                ...
                const kneePos = V3.lerp(foot.position, socketPosWorld, 0.5);
                kneePos[2] += 2;
                const segs = [
...
//# sourceMappingURL=summon-monster.js.map