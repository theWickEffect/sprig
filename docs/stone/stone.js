...
import { mat4, V, V3, quat } from "../matrix/sprig-matrix.js";
...
function shrinkBrickAtIndex(stone, baseIndex, aabb) {
    ...
    for (let index of face) {
        ...
        if (pointInAABB(aabb, attracted)) {
            ...
            if (pointInAABB(aabb, V3.add(attracted, V3.scale(towardsAttractor, max, testAABBTmp), testAABBTmp))) {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                if (pointInAABB(aabb, V3.add(attracted, V3.scale(towardsAttractor, half, testAABBTmp), testAABBTmp))) {
                    min = half;
                }
                else {
                    max = half;
...
// takes a tower-space AABB--not world space!
function knockOutBricksByBullet(tower, bricks, aabb, bullet) {
...
function createTowerState() {
    ...
    const windowAABB = {
        min: V(-approxBrickWidth, windowHeight - 2 * brickHeight, -baseRadius - 4 * brickDepth),
        max: V(approxBrickWidth, windowHeight + 2 * brickHeight, -baseRadius + 4 * brickDepth),
    };
...
export const TowerPoolDef = EM.defineResource("towerPool", (p) => p);
        ...
        onSpawn: (p) => {
            ...
            if (restoreAllBricks(p.stoneTower)) {
                EM.whenEntityHas(p, RenderableDef).then((e) => {
                    res.renderer.renderer.stdPool.updateMeshVertices(e.renderable.meshHandle, p.stoneTower.stone.mesh);
                });
            }
...
export const FlyingBrickPoolDef = EM.defineResource("flyingBrickPool", (p) => p);
EM.addLazyInit([RendererDef], [FlyingBrickPoolDef], (res) => {
    const flyingBrickPool = createEntityPool({
        max: maxFlyingBricks,
        ...
        onSpawn: (e) => {
            ...
            V3.set(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5, e.angularVelocity);
            V3.scale(e.angularVelocity, 0.01, e.angularVelocity);
            V3.set(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5, e.linearVelocity);
...
EM.addSystem("despawnFlyingBricks", Phase.GAME_WORLD, [FlyingBrickDef, DeadDef], [FlyingBrickPoolDef], (es, res) => es.forEach((e) => {
    if (!e.dead.processed) {
...
EM.addSystem("stoneTowerAttack", Phase.GAME_WORLD, [StoneTowerDef, WorldFrameDef, RotationDef], [TimeDef, PartyDef], (es, res) => {
    ...
    for (let tower of es) {
        ...
        const targetVel = V3.scale(V3.sub(res.party.pos, __previousPartyPos), 1 / (res.time.time - __prevTime));
        ...
        // fire bullet
        const b = fireBullet(2, cannon.world.position, worldRot, tower.stoneTower.projectileSpeed, 0.02, GRAVITY, 
        // 2.0,
        20.0, 
...
function destroyTower(tower, bricks) {
    let knockedOut = 0;
    let i = 0;
...
EM.addSystem("stoneTowerDamage", Phase.GAME_WORLD, [StoneTowerDef, RenderableDef, WorldFrameDef], [PhysicsResultsDef, RendererDef, FlyingBrickPoolDef], (es, res) => {
    ...
    for (let tower of es) {
        ...
        if (hits) {
            ...
            for (let ball of balls) {
                ...
                totalKnockedOut += knockOutBricksByBullet(tower, res.flyingBrickPool, ballAABB, ball.bullet);
            }
            if (totalKnockedOut) {
                EM.whenResources(AudioDef, SoundSetDef).then((res) => {
