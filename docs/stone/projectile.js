...
import { V, V2, V3, mat4, quat, tmpStack } from "../matrix/sprig-matrix.js";
import { createAABB, createAABB2, getAABBFromPositions, } from "../physics/aabb.js";
import { OBB, getOBBCornersTemp, getRandPointInOBB } from "../physics/obb.js";
import { angularDiff } from "../utils/math.js";
import { sketchFan, sketchLine, sketchPoints, sketchYawPitch, } from "../utils/sketch.js";
import { compileSVG } from "../utils/svg.js";
...
function projectOntoPlane(v, p, out) {
    throw "TODO";
}
// TODO(@darzu): ridiculous? maybe.
export function getAABB2CircleSweptPerimeterAsSvg(aabb, radius) {
...
export function getAimAndMissPositions(opt, out) {
    out = out ?? V3.tmp();
    if (opt.doMiss) {
        const _stk = tmpStack();
        ...
        const localAABB2 = createAABB2(V(localAABB.min[0], localAABB.min[2]), V(localAABB.max[0], localAABB.max[2]));
        ...
        // back to world frame
        const to3d = (v2d, out) => V3.tMat4([v2d[0], 0, v2d[1]], localToWorldM, out);
...
function getApproxFlightTime({ projectileSpeed: v, targetOBB, sourcePos, }) {
    // calculate initial distance
    const d0 = V3.dist([sourcePos[0], sourcePos[1], 0], [targetOBB.center[0], targetOBB.center[1], 0]);
...
export function getFireSolution(opt) {
    assert(opt.gravity >= 0, `Gravity should be positive. TODO: support different directions of gravity.`);
    // get flight time
    ...
    V3.add(opt.targetOBB.center, V3.scale(opt.targetVel, approxFlightTime), opt.targetOBB.center);
    ...
    const pitch1Valid = !isNaN(pitch1) && pitch1 <= opt.maxPitch && opt.minPitch <= pitch1;
    const pitch2Valid = !isNaN(pitch2) && pitch2 <= opt.maxPitch && opt.minPitch <= pitch2;
    const pitch = pitch1Valid && pitch2Valid
    ...
    if (DBG_AIM_POS) {
        const sketchPitch = (p, k, c) => sketchYawPitch(sourcePos, srcBaseYaw, p, {
            length: 100,
            key: k,
            color: c,
        });
        if (!isNaN(pitch1))
            sketchPitch(pitch1, "pitch1", ENDESGA16.midBrown);
        if (!isNaN(pitch2))
            sketchPitch(pitch2, "pitch2", ENDESGA16.lightBrown);
        if (!isNaN(pitch2))
...
export function getFirePitches(aimPos, { sourcePos, projectileSpeed: v, gravity: g }) {
