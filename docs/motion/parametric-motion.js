...
import { V3, V, quat } from "../matrix/sprig-matrix.js";
...
export const ParametricDef = EM.defineComponent("parametric", createParametric, (p, init) => {
    if (init) {
...
export function copyParamateric(out, p) {
...
export function createPathFromParameteric(p, deltaT, out) {
    // get positions
...
EM.addEagerInit([ParametricDef], [], [], () => {
    EM.addSystem("updateParametricMotion", Phase.PHYSICS_MOTION, [PositionDef, ParametricDef], [TimeDef], (es, res) => {
        for (let e of es) {
            projectilePosition(e.parametric.pos, e.parametric.vel, e.parametric.accel, res.time.time - e.parametric.time, e.position);
        }
    });
});
