...
*/
// TODO(@darzu): CANVAS
export const CanvasDef = EM.defineResource("htmlCanvas", // TODO(@darzu): rename to canvas ?
T());
...
EM.addLazyInit([], [CanvasDef], () => {
    ...
    const canvasesByName = toMap(canvasNames, (n) => n, (_, i) => canvases[i]);
    ...
    if (VERBOSE_LOG)
        console.log(`preferred device pixel ratio: ${window.devicePixelRatio} vs ours: ${comp.pixelRatio}`);
    console.log(`initial canvas size: ${window.innerWidth * comp.pixelRatio} x ${window.innerHeight * comp.pixelRatio}\n${((window.innerWidth *
        comp.pixelRatio *
        window.innerHeight *
        comp.pixelRatio) /
        1_000_000).toFixed(1)}MP`);
    function setActiveCanvasSize(width, height) {
