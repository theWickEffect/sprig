...
*/
const VORONOI_FORMAT = "rg16uint";
const SDF_FORMAT = "r16float";
...
export function createJfaPipelines({ name, maskTex, maskMode, maxDist, shader, shaderExtraGlobals, sdfDistFact, size, sizeToCanvas, stepAscending, }) {
    size = size ?? 512;
    ...
    if (maskMode === "border") {
        assert(!VORONOI_FORMAT.endsWith("int"), `TODO: support border JFA w/ VORONOI_FORMAT: ${VORONOI_FORMAT}`);
        extractUvMaskShader = () => `
  ...
  `;
    ...
    assert(!maxDist || Math.log2(maxDist) % 1 === 0, `maxDist: ${maxDist} must be power of two`);
