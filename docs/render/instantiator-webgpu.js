...
import { capitalize, pluralize, uncapitalize, isString, isFunction, } from "../utils/util.js";
...
import { isResourcePtr, isRenderPipelinePtr, meshPoolPrimToTopology, numIndsPerPrim, } from "./gpu-registry.js";
...
export function bundleRenderPipelines(device, resources, renderPipelines, meshHandleIds) {
    ...
    for (let p of renderPipelines) {
        ...
        if (p.ptr.meshOpt.stepMode === "per-instance") {
            ...
            assert(meshPoolPrimToTopology[poolPrim] === pipeTopo, `topology mismatch`);
            ...
            for (let set of p.pool.sets) {
                ...
                for (let m of set.meshes) {
                    ...
                    bundleEnc.drawIndexed(numPrim * indsPer, undefined, m.primIdx * indsPer, m.vertIdx);
...
export function startBundleRenderer(getCanvasTexture, commandEncoder, resources) {
    // render bundles
    ...
    let _lastCanvasTexture = undefined;
    let _lastCanvasView = undefined;
...
export function onCanvasResizeAll(resources, canvasSize) {
    for (let tex of [
