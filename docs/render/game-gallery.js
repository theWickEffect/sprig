...
export async function initGalleryGame() {
    ...
    EM.addSystem("galleryGamePipelines", Phase.GAME_WORLD, null, [RendererDef, DevConsoleDef], (_, res) => {
        // renderer
    ...
    if (SHOW_SKYDOME) {
        ...
        EM.set(sky, RenderableConstructDef, skyMesh
        // undefined,
        // undefined,
        // SKY_MASK
        );
...
async function createGallery() {
    ...
    function presentGameMesh(m) {
        ...
        EM.set(obj, PositionDef, V(x - m.center[0] * scale, -m.center[1] * scale, -m.aabb.min[2] * scale));
        if (hasScale)
            EM.set(obj, ScaleDef, V(scale, scale, scale));
        ...
        if (!anyColor)
            EM.set(obj, ColorDef, ENDESGA16.lightGray);
        lastX = x + halfsize * scale;
    }
    const { renderer, sg_meshes } = await EM.whenResources(RendererDef, shadingGameMeshesDef);
...
//# sourceMappingURL=game-gallery.js.map