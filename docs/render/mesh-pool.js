...
import { CY, numIndsPerPrim, } from "./gpu-registry.js";
...
function logMeshPoolStats(pool) {
    ...
    if (VERBOSE_MESH_POOL_STATS) {
        ...
        console.log(`Mesh space usage for up to ${maxMeshes} meshes, ${maxPrims} tris/prims, ${maxVerts} verts:`);
        console.log(`   ${((maxVerts * vertStruct.size) / 1024).toFixed(1)} KB for verts`);
        const bytesPerPrim = Uint16Array.BYTES_PER_ELEMENT * numIndsPerPrim(pool.ptr.prim);
        console.log(`   ${((maxPrims * bytesPerPrim) / 1024).toFixed(1)} KB for prim indices`);
...
function getPointInds(m, startIdx, count) {
    ...
    if (tempPointData.length < dataLen)
        tempPointData = new Uint16Array(dataLen);
    // add points
...
function getLineInds(m, startIdx, count) {
    // NOTE: callee responsible for aligning-up the output length
    // NOTE: caller responsible for aligning-down start-idx
    assertDbg(startIdx % 2 === 0);
    ...
    // expand our temp array if needed
    if (tempLineData.length < dataLen)
        tempLineData = new Uint16Array(dataLen);
    // add lines
...
function getTriInds(m, startIdx, count) {
    // NOTE: callee responsible for aligning-up the output length
    // NOTE: caller responsible for aligning-down start-idx
    assertDbg(startIdx % 2 === 0);
    // assert(count % 2 === 0);
...
function getQuadInds(m, startIdx, count) {
...
device, resources, ptr) {
    ...
    function getNumPrimsOfKind(m, k) {
        if (primKind === "tri")
            return m.tri.length + m.quad.length * 2;
    ...
    function addMesh(m, reserved) {
        ...
        if (primKind === "tri") {
            if (m.quad.length)
                updateMeshQuadInds(handle, m);
            if (m.tri.length)
                updateMeshTriInds(handle, m);
        }
        else if (primKind === "line") {
            if (m.lines?.length)
                updateMeshLineInds(handle, m);
        }
        else if (primKind === "point") {
    ...
    function updateMeshPointInds(handle, newMesh, pointIdx, pointCount) {
        ...
        if (PERF_DBG_GPU)
            _stats._accumPrimDataQueued += pointData.byteLength;
    }
    function updateMeshLineInds(handle, newMesh, lineIdx, lineCount) {
        ...
        if (PERF_DBG_GPU)
            _stats._accumPrimDataQueued += lineData.byteLength;
    }
    function updateMeshTriInds(handle, newMesh, triIdx, triCount) {
        assert(primKind === "tri");
        triIdx = triIdx ?? 0;
      ...
      newMesh.tri.length: ${newMesh.tri.length}`);
        ...
        if (PERF_DBG_GPU)
            _stats._accumPrimDataQueued += triData.length * 2.0;
    }
    function updateMeshQuadInds(handle, newMesh, quadIdx, quadCount) {
        assert(primKind === "tri");
        quadIdx = quadIdx ?? 0;
        ...
        const bufQuadIndsStart = align((handle.primIdx + newMesh.tri.length) * 3, 2); // NOTE: tris come first
        ...
        if (PERF_DBG_GPU)
            _stats._accumPrimDataQueued += quadData.byteLength;
    }
    function updateMeshSize(handle, newMesh) {
        ...
        assert(newNumPrim <= handle.reserved.maxPrimNum, "Too many triangles/primatives!");
        // TODO(@darzu): what to do about this requirement...
        assert(!m.quad.length || m.tri.length % 2 === 0, `tri.length not even for ${m.dbgName}`);
