...
import { LOG_WEBGPU_AVAILABLE_FEATURES, PERF_DBG_GPU, PERF_DBG_REBUNDLE, VERBOSE_LOG, } from "../flags.js";
...
export function createRenderer(device, shaders, canvas) {
    ...
    function submitPipelines(handles, pipelinePtrs) {
        ...
        const didRenderPipelinesChange = lastRenderPipelines.length !== renderPipelines.length ||
            lastRenderPipelines.reduce((p, n, i) => p || lastRenderPipelines[i].ptr.name !== renderPipelines[i].ptr.name, false);
        ...
        const bundleRenderer = startBundleRenderer(canvas.getCanvasTexture, commandEncoder, resources);
        // run pipelines
