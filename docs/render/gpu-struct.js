...
function wgslTypeToDummyVal(wgsl) {
    ...
    function _wgslTypeToDummyVal(wgsl) {
        ...
        if (wgsl === "vec3<f32>")
            return randVec3();
        const randVec4 = () => V(Math.random(), Math.random(), Math.random(), Math.random());
        if (wgsl === "vec4<f32>")
            return randVec4();
        if (wgsl === "vec4<u32>")
            return V(Math.floor(Math.random() * Number.MAX_SAFE_INTEGER), Math.floor(Math.random() * Number.MAX_SAFE_INTEGER), Math.floor(Math.random() * Number.MAX_SAFE_INTEGER), Math.floor(Math.random() * Number.MAX_SAFE_INTEGER));
        if (wgsl === "u32")
            return Math.floor(Math.random() * Number.MAX_SAFE_INTEGER);
        const randAngle = () => Math.random() * 2 * Math.PI;
        const randQuat = () => quat.fromEuler(randAngle(), randAngle(), randAngle(), quat.mk());
        if (wgsl === "mat4x4<f32>")
...
export function cloneStruct(desc, data) {
    let res = {};
...
export function createStruct(desc) {
...
export function checkStruct(desc, data) {
    ...
    for (let name of descKeys) {
        assert(checkValue(desc[name], data[name]), `bad struct val {${name}: ${desc[name]} = ${data[name]}}`);
    }
    return true;
}
    ...
    function fromPartial(p) {
        ...
        for (let name of Object.keys(struct.desc)) {
            if (p[name] !== undefined)
                ...
                res[name] = cloneValue(struct.desc[name], _default[name]);
        }
        // console.dir(partial);
        // console.dir(res);
        return res;
