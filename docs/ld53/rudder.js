...
export function createRudderTurret(res) {
    ...
    constructNetTurret(rudder, 0, 0, interactBox, 0.0 * Math.PI, 
    // -Math.PI / 8,
    -Math.PI / 12, 1.6, 
    // V(0, 20, 50),
    V(0, -30, 10), // camera offset
    true, 1, Math.PI, "W/S: unfurl/furl sail, A/D: turn, E: drop rudder");
...
EM.addEagerInit([RudderDef], [], [], () => {
    ...
    EM.addSystem("easeRudder", Phase.GAME_WORLD, [RudderDef, YawPitchDef], [MeDef], (rudders, res) => {
        for (let r of rudders) {
            if (AuthorityDef.isOn(r) && r.authority.pid !== res.me.pid)
                continue;
            if (TurretDef.isOn(r) && r.turret.mannedId !== 0)
                continue;
    ...
    EM.addSystem("rudderTurn", Phase.GAME_PLAYERS, [HasRudderDef, RotationDef], [], (es) => {
        for (let e of es) {
            // rudder
