...
import { createAABB, getSizeFromAABB, mergeAABBs, transformAABB, } from "../physics/aabb.js";
...
let __dbgLastLandGraph = undefined;
EM.addSystem("landShipCollision", Phase.GAME_WORLD, [LD52ShipDef, PositionDef, WorldFrameDef, ColliderDef, LinearVelocityDef], [PartyDef, LandDef, LevelMapDef], (es, res) => {
    if (!es.length)
        return;
    const ship = es[0];
    ...
    V2.set(res.party.dir[0] * halfLength, res.party.dir[1] * halfLength, yBasis);
    const UP = [0, 0, 1];
    ...
    const corners = cornersFromCenter.map((v) => V2.add(shipCenter, v));
    //console.log(corners);
    ...
    if (DBG_COLLISIONS &&
        ...
        for (let i = 0; i < corners.length; i++) {
            ...
            const ball = drawBall([localCorner[0], localCorner[1], 0], scale, ENDESGA16.lightGreen);
            ...
            for (let s = 1; s <= SAMPLES_PER_EDGE; s++) {
                ...
                const ball = drawBall([localPoint[0], localPoint[1], 0], scale, ENDESGA16.red);
                EM.set(ball, PhysicsParentDef, ship.id);
            }
        }
        ...
        const graph = createGraph3D(V(-halfWidth * 1, -halfHeight * 1, 0), samples, ENDESGA16.red, createAABB(V(-halfWidth, -halfHeight, 0), V(halfWidth, halfHeight, 1)), createAABB(V(-halfWidth * 1, -halfHeight * 1, 0), V(halfWidth * 1, halfHeight * 1, 20)));
    ...
    for (let i = 0; i < corners.length; i++) {
        ...
        for (let s = 1; s <= SAMPLES_PER_EDGE; s++) {
            ...
            if (isLand(point[0], point[1])) {
                ...
                const nudge = V3.cross(UP, V3.set(dist[0], dist[1], 0, nudgeTemp), nudgeTemp);
                V3.norm(nudge, nudge);
                V3.scale(nudge, NUDGE_DIST, nudge);
                if (DBG_COLLISIONS)
                    drawVector(nudge, {
