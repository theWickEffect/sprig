...
import { createMast, HasMastObj } from "../wind/mast.js";
import { ColliderDef, } from "../physics/collider.js";
import { TurretDef } from "../turret/turret.js";
...
import { HasRudderObj, createRudderTurret } from "./rudder.js";
...
export async function createLd53ShipAsync() {
    ...
    // create cannons
    const cannonR = createCannonNow(res, V(8, -7, 4.7), 0.5 * Math.PI, cannonDefaultPitch);
    V3.copy(cannonR.color, ENDESGA16.darkGray);
    const cannonL = createCannonNow(res, V(-8, -7, 4.7), -0.5 * Math.PI, cannonDefaultPitch);
...
EM.addSystem("shipParty", Phase.GAME_WORLD, [LD52ShipDef, PositionDef, RotationDef, ColliderDef, WorldFrameDef], [PartyDef], (es, res) => {
    if (!es.length)
        return;
    const ship = es[0];
    ...
    const localAABB = createAABB(tV(+Infinity, +Infinity, +Infinity), tV(-Infinity, -Infinity, -Infinity));
