...
import { BallMesh, CubeMesh, PirateMesh, } from "../meshes/mesh-list.js";
...
import { quat, tV, V, V2, V3 } from "../matrix/sprig-matrix.js";
import { DevConsoleDef } from "../debug/console.js";
import { jitter, max } from "../utils/math.js";
...
import { LD52ShipDef, cannonDefaultPitch, createLd53ShipAsync, } from "./ship.js";
...
const worldXToTexX = (x) => Math.floor(x + WORLD_WIDTH / 2);
const worldYToTexY = (y) => Math.floor(y + WORLD_HEIGHT / 2);
const texXToWorldX = (x) => x + 0.5 - WORLD_WIDTH / 2;
const texYToWorldY = (y) => y + 0.5 - WORLD_HEIGHT / 2;
const level2DtoWorld3D = (levelPos, z, out) => V3.set(texXToWorldX(levelPos[0]), texYToWorldY(levelPos[1]), z, out);
...
async function hostResetLevel(levelIdx) {
    ...
    const ship = await EM.whenSingleEntity(PositionDef, RotationDef, LD52ShipDef, HasMastDef, HasRudderDef, LinearVelocityDef, ShipHealthDef, WoodHealthDef, WoodStateDef);
    ...
    EM.whenEntityHas(ship, RenderableDef, WoodStateDef).then((ship) => renderer.renderer.stdPool.updateMeshQuadInds(ship.renderable.meshHandle, ship.woodState.mesh, 0, ship.woodState.mesh.quad.length));
    // reset dock
    ...
    EM.whenEntityHas(dock, RenderableDef, WoodStateDef).then((dock) => renderer.renderer.stdPool.updateMeshQuadInds(dock.renderable.meshHandle, dock.woodState.mesh, 0, dock.woodState.mesh.quad.length));
}
async function setLevelLocal(levelIdx) {
    ...
    await resetLand();
    const { levelMap, towerPool } = await EM.whenResources(LevelMapDef, TowerPoolDef);
    ...
    // spawn towers
    const towerPosAndYaw = levelMap.towers.map(([tPos, tDir]) => [
...
export async function initLD53() {
    ...
    if (DBG_PLAYER) {
        ...
        EM.addSystem("smolGhost", Phase.GAME_WORLD, [GhostDef, WorldFrameDef, ColliderDef], [InputsDef, HasFirstInteractionDef], (ps, { inputs }) => {
            if (!ps.length)
                return;
            const ghost = ps[0];
        });
    ...
    copyAABB(res.camera.maxWorldAABB, createAABB(V(-WORLD_WIDTH * 1.1, -WORLD_HEIGHT * 1.1, -100), V(WORLD_WIDTH * 1.1, WORLD_HEIGHT * 1.1, 100)));
    // console.dir(mapJfa);
    ...
    const wingAngle = Math.atan2(level.levelMap.windDir[1], level.levelMap.windDir[0]);
    ...
    */
            ...
            if (rudder.turret.mannedId) {
                if (MOTORBOAT_MODE) {
                    ...
                    if (res.inputs.keyDowns["w"]) {
                        V3.add(ship.linearVelocity, V3.scale(res.party.dir, 0.1), ship.linearVelocity);
                    }
                }
                else {
    ...
    EM.whenSingleEntity(LD52ShipDef, HasRudderDef, FinishedDef).then(async (ship) => {
        // player
        if (!DBG_PLAYER) {
    ...
    // world gizmo
    if (DBG_PLAYER)
        addWorldGizmo(V(-WORLD_WIDTH / 2, -WORLD_HEIGHT / 2, 0), 100);
    ...
    EM.addSystem("deadBullets", Phase.POST_GAME_WORLD, [BulletDef, PositionDef, DeadDef, RenderableDef], [], (es, _) => {
        for (let e of es) {
            if (e.dead.processed)
                continue;
            e.bullet.health = 10;
...
async function resetLand() {
    ...
    function sampleTerra(worldX, worldY) {
        let xi = ((worldX + WORLD_WIDTH * 0.5) / WORLD_WIDTH) * terraReader.size[0];
        let yi = ((worldY + WORLD_HEIGHT * 0.5) / WORLD_HEIGHT) * terraReader.size[1];
