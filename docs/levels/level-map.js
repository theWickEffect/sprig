...
const WIDTH = 1024; // +x
const HEIGHT = 512; // +y
function mapXYtoImgPxDataIdx(x, y, width, height) {
    // NOTE:
    //  image origin is top-left
    //  map origin is bottom-left
    return x + (height - 1 - y) * width;
}
function mapXYtoCyTexDataIdx(x, y, width, height) {
...
function parseAndMutateIntoMapBlobs(rgbaBytes, width, height) {
    ...
    function parseBlob(x, y) {
        ...
        const checkColor = (x, y) => V4.equals(color, getColor(x, y));
        parseRun(x, y);
...
export function parseAndMutateIntoMapData(mapBytes, name) {
    ...
    if (DBG_PRINT_BLOBS)
        for (let b of blobs)
            console.log(`xy: ${vec2Dbg(aabbCenter2(tV(0, 0), b.aabb))}, clr: ${vec4Dbg(b.color)}, area: ${b.area}`);
    const W = 2;
        ...
        if (blob.color[0] < 50 && blob.color[1] < 50 && blob.color[2] < 50) {
            for (let r of blob.runs) {
                for (let x = r.x0; x < r.x1; x++) {
                    ...
                    const outIdx = mapXYtoCyTexDataIdx(x, r.y, mapBytes.width, mapBytes.height);
    ...
    towers.forEach(([pos, _]) => {
        for (let i = -towerIslandRadius; i < towerIslandRadius; i++) {
            for (let j = -towerIslandRadius; j < towerIslandRadius; j++) {
                ...
                const outIdx = mapXYtoCyTexDataIdx(x, y, mapBytes.width, mapBytes.height);
