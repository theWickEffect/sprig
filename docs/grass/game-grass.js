...
import { mat3, mat4, quat, V, V3 } from "../matrix/sprig-matrix.js";
...
const level2DtoWorld3D = (levelPos, y, out) => V3.set(texYToWorldX(WORLD_HEIGHT - 1 - levelPos[1]), y, texXToWorldZ(levelPos[0]), out);
export async function initGrassGame() {
    ...
    EM.addSystem("cutGrassUnderShip", Phase.GAME_WORLD, [LD52ShipDef, PositionDef, WorldFrameDef, PhysicsStateDef], [PartyDef, ScoreDef], (es, res) => {
        if (!es.length)
            return;
        const ship = es[0];
