...
import { HEX_AABB, mkCubeMesh, mkRectMesh } from "../meshes/primatives.js";
...
export async function initJoelGame() {
  ...
  const wall = EM.mk();
  EM.set(wall, RenderableConstructDef, mkRectMesh(10,3,20));
  ...
  const hold1 = EM.mk();
  EM.set(hold1, RenderableConstructDef, TetraMesh);
  EM.set(hold1, ColorDef, ENDESGA16.darkGreen);
  EM.set(hold1, PositionDef, V(0,0,10));
  EM.set(hold1,RotationDef, quat.fromYawPitchRoll(0,Math.PI*.6,0));
